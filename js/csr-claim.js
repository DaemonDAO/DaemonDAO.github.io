...
const rpc = 'https://mainnode.plexnode.org:8545';
...
async function checkCSRBalance(){
  ...
  let balcorr = (Math.round(Number(balance)/1e18 * 10000) / 10000).toFixed(4);
...
async function claimCSR() {
    ...
    let balance = await turnContract.methods.balances(tokenId).call();
    console.log(balance);
    //let balance_wei = web3.utils.toWei((balance/1e18).toString(), 'ether');
    let withdraw = turnContract.methods.withdraw(tokenId, selectedAccount, balance);
...
async function populateNFTs(address) {
  ...
  if (tokenList.length > 0) {
    ...
    galleryCode += `<h3><img src="./images/dancers.gif" alt="cheers"/></h3>`;
